 ...
 */
final class AuthEventsAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private AuthEventRepository $authEventRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'auth_events' => $this->authEventRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\AuthEvents\ModuleException("Record {$idStr} not found for locking.");
            }
