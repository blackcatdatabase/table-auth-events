...
use BlackCat\Database\Packages\AuthEvents\Dto\AuthEventDto as Dto;
use BlackCat\Database\Packages\AuthEvents\Mapper\AuthEventDtoMapper as RowMapper;
...
final class AuthEventRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\AuthEvents\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\AuthEvents\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\AuthEvents\Dto\AuthEventDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\AuthEvents\Dto\AuthEventDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\AuthEvents\Mapper\AuthEventDtoMapper::fromRow($row);
    }
