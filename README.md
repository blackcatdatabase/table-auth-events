...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 1 outbound / 1 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 5 · Changelog: fresh (2 d)</div>
</div>
...
- Pkg folder: [packages\auth-events](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\auth-events)
...
graph LR
  ...
  auth_events["auth_events"]:::center
  auth_events -->|FK| users["users"]:::link
  login_attempts["login_attempts"]:::inbound -->|FK| auth_events
...
- Inbound (relies on this): "login_attempts"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package auth-events -Apply
